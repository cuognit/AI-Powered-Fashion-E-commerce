...
export default function OrderHistory() {
  ...
  return <main className='min-h-[75vh] bg-[#f4f2ed] py-10'><div className='mx-auto max-w-[1360px] px-4 sm:px-6 lg:px-8'>
    ...
    {loading ? <div className='h-56 animate-pulse bg-white' /> : error ? <div className='bg-white p-10 text-center'><AlertCircle className='mx-auto' /><p className='mt-3'>{error}</p><button onClick={load} className='mt-4 bg-black px-5 py-3 text-xs font-black uppercase text-white'>Thử lại</button></div> : !orders.length ? <div className='bg-white p-12 text-center'><PackageOpen className='mx-auto h-12 w-12 text-neutral-400' /><h2 className='mt-4 text-xl font-black uppercase'>Chưa có đơn hàng</h2><Link to='/collections' className='mt-5 inline-block bg-black px-6 py-3 text-xs font-black uppercase text-white'>Bắt đầu mua sắm</Link></div> : <div className='space-y-5'>{orders.map((order) => <article key={order.orderCode} className='bg-white p-5 shadow-sm sm:p-7'>
      ...
      <div className='grid gap-6 py-5 md:grid-cols-[1fr_220px]'><OrderItemList items={order.items} compact /><div className='bg-neutral-50 p-4 text-xs'><div className='flex justify-between'><span>Thanh toán</span><b>{paymentStatusLabels[order.paymentStatus]}</b></div>{order.refund?.status !== 'none' && <div className='mt-2 flex justify-between text-amber-700'><span>Hoàn tiền</span><b>{refundStatusLabels[order.refund.status]}</b></div>}<div className='mt-3 flex justify-between border-t pt-3 text-sm'><span>Tổng cộng</span><b>{formatCurrency(order.totalAmount)}</b></div></div></div>
      <div className='flex flex-wrap justify-end gap-2 border-t pt-4'><button disabled={busyCode === order.orderCode} onClick={() => handleReorder(order.orderCode)} className='flex items-center gap-2 border px-4 py-2.5 text-xs font-black uppercase disabled:opacity-50'><RotateCcw className='h-4 w-4' />Mua lại</button>{['pending', 'processing'].includes(order.status) && <button onClick={() => setCancelTarget(order)} className='border border-red-200 px-4 py-2.5 text-xs font-black uppercase text-red-600'>Hủy đơn</button>}{order.status === 'shipped' && <button disabled={busyCode === order.orderCode} onClick={() => handleReceived(order.orderCode)} className='flex items-center gap-2 bg-emerald-700 px-4 py-2.5 text-xs font-black uppercase text-white disabled:opacity-50'><CheckCircle2 className='h-4 w-4' />Đã nhận hàng</button>}<Link to={`/track-order?orderCode=${encodeURIComponent(order.orderCode)}`} className='flex items-center gap-2 bg-black px-4 py-2.5 text-xs font-black uppercase text-white'><Truck className='h-4 w-4' />Theo dõi</Link></div>
    </article>)}</div>}
