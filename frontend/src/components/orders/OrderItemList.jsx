...
export default function OrderItemList({ items = [], compact = false }) {
  ...
  return <div className='divide-y divide-neutral-100'>
    {visible.map((item) => <div key={item.variantSku} className='flex gap-4 py-4 first:pt-0 last:pb-0'>
      <div className='h-20 w-16 shrink-0 overflow-hidden bg-neutral-100'>
        {item.image ? <img src={item.image} alt={item.name} className='h-full w-full object-cover' /> : <div className='grid h-full place-items-center text-[9px] font-bold uppercase text-neutral-400'>No image</div>}
      </div>
