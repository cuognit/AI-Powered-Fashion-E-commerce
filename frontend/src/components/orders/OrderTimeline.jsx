import { Check, ClipboardCheck, PackageCheck, PackageOpen, RotateCcw, ShoppingBag, Truck, X } from 'lucide-react'
import { formatOrderDate } from '../../utils/orderStatus.js'

const steps = [
  ['order_created', 'Chờ xác nhận', ShoppingBag],
  ['processing', 'Chờ xử lý đơn', ClipboardCheck],
  ['ready_to_ship', 'Chờ vận chuyển', PackageOpen],
  ['shipped', 'Đang giao', Truck],
  ['completed', 'Đã giao', PackageCheck],
]
const statusIndex = { pending: 0, processing: 1, ready_to_ship: 2, shipped: 3, completed: 4 }
...
function CanceledTimeline({ order, history }) {
  ...
  return <div className='grid gap-4 sm:grid-cols-2'>
    <div className='flex gap-3 border p-4'><Check /><div><b>Đã đặt hàng</b><p>{formatOrderDate(order.createdAt, true)}</p></div></div>
    <div className='flex gap-3 border border-red-200 bg-red-50 p-4'><X /><div><b>Đơn hàng đã hủy</b><p>{formatOrderDate(canceledAt, true)}</p></div></div>
    {order.refund?.status !== 'none' && <div className='flex gap-3 border border-amber-200 bg-amber-50 p-4 sm:col-span-2'>
...
export default function OrderTimeline({ order }) {
  ...
  return <div className='relative grid gap-3 md:grid-cols-5'>
    <div className='absolute left-[10%] right-[10%] top-6 hidden h-px bg-neutral-200 md:block' />
    {steps.map(([key, label, Icon], index) => {
      const reached = index <= activeIndex
      const current = index === activeIndex && order.status !== 'completed'
      return <div key={key} className={`relative flex gap-4 border p-4 md:flex-col md:items-center md:border-0 md:bg-transparent md:p-0 md:text-center ${reached ? 'border-black bg-white' : 'border-neutral-200 bg-neutral-50 text-neutral-400'}`}>
