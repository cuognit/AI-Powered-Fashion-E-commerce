...
import { allocateReorderQuantity, allowedAdminTransitions, escapeRegex, legacyStatusHistory } from '../src/utils/order.js'
...
test('admin order transitions are forward-only and terminal states stay terminal', () => {
  assert.deepEqual(allowedAdminTransitions('pending'), ['processing', 'canceled'])
  assert.deepEqual(allowedAdminTransitions('processing'), ['shipped', 'canceled'])
  assert.deepEqual(allowedAdminTransitions('shipped'), ['completed'])
